...
dotenv.config();
const app = express();
app.use(express.json({ limit: '50mb' }));  // Parse application/json with larger payload
app.use(express.urlencoded({ extended: true, limit: '50mb' }));  // Parse application/x-www-form-urlencoded with larger payload

// Static file serving (if needed for the assets or views)
...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        ...
        if (!productTitle) {
            return cb(new Error('Product title is required.'));
        }

        const productDir = path.join(__dirname, '../Frontend/assets/ProductsImages', productTitle);
...
}).array('images'); // Expecting multiple images
...
app.post('/upload-product', (req, res) => {
    upload(req, res, (err) => {
        if (err) {
            ...
            return res.status(500).send(err.message || 'Error uploading images.');
        }
...
app.delete('/delete-product-img/:productTitle', (req, res) => {
    const { productTitle } = req.params;
    const productDir = path.join(__dirname, '../Frontend/assets/ProductsImages', productTitle);

    fs.access(productDir, fs.constants.F_OK, (err) => {
        if (err) {
            console.error('Directory not found:', err);
            return res.status(404).send('Product directory not found.');
        }
        ...
        fs.rm(productDir, { recursive: true, force: true }, (err) => {
            if (err) {
                ...
                return res.status(500).send('Error deleting product directory.');
            }

            res.send('Product folder and all associated images deleted successfully!');
        });
    });
});
...
app.post('/upload-category', (req, res) => {
    ...
    if (!image || !filename) {
        return res.status(400).send('No category image or filename provided.');
    }
    const imageBuffer = Buffer.from(image, 'base64');
        if (imageBuffer.length > 1024 * 1024) {
        return res.status(400).send('Image size exceeds limit of 500 KB.');
    }
    const filePath = path.join(__dirname, '../Frontend/assets/imgs/shop', filename);
    fs.writeFile(filePath, imageBuffer, err => {
        if (err) {
            console.error('Error saving category image:', err);
            return res.status(500).send('Error saving category image.');
        }
        res.send('Image of category saved successfully!');
    });
});


app.delete('/delete-category-img/:filename', (req, res) => {
    ...
    fs.access(filePath, fs.constants.F_OK, (err) => {
        ...
        fs.unlink(filePath, (err) => {
            ...
            res.send('Product and associated image deleted successfully!');
        });
    });
});

//THESE ROUTES ARE FOR USERS AND NOT FOR DASHBOARD
...
<body>
                        ...
                        <div class="header-action-right">
                            <div class="header-action-2">
                                ...
                                <div class="header-action-icon-2">
                                    ...
                                    <a href="http://127.0.0.1:3000/Nest-Users/frontend/shop-cart.html"><span class="lable">Cart</span></a>
        ...
        <div class="container mb-30">
            <div class="row">
                <div class="col-lg-4-5">
                    ...
                    <div class="row product-grid" id="shop-grid-right-product-container">
   ...
   <script>
    document.addEventListener("DOMContentLoaded", () => {
      ...
      const path = window.location.pathname; // e.g., '/shop-grid-right/electronics'
      const category = path.split("/").pop(); // 'electronics'
console.log(category)
      // Send the category to the backend to fetch products
      fetch('http://localhost:8000/products/getProducts/' + category, {
        method: "GET",
        headers: {
    ...
    const renderProducts = (products) => {
      const productContainer = document.getElementById("shop-grid-right-product-container");
      if (!productContainer) {
      ...
      products.forEach((product) => {
        const formattedTitle = product.ProductTitle.replace(/\\s+/g, "-"); // replace spaces with hyphens
        const productDiv = document.createElement("div");
        productDiv.classList = "col-lg-1-5 col-md-4 col-12 col-sm-6";
        productDiv.innerHTML = \`
            <div class="product-cart-wrap mb-30">
              <div class="product-img-action-wrap">
                <div class="product-img product-img-zoom">
                  <a href="shop-product-right.html">
                    <img class="default-img" src="http://localhost:8000/ProductsImages/\${formattedTitle}.jpg" alt="Err1" />
                    <img class="hover-img" src="http://localhost:8000/ProductsImages/\${formattedTitle}.jpg" alt="Err2" />
                  </a>
                </div>
                <div class="product-action-1">
                  <a aria-label="Add To Wishlist" class="action-btn" href="shop-wishlist.html"><i class="fi-rs-heart"></i></a>
              ...
              <div class="product-content-wrap">
                ...
                <h2><a href="shop-product-right.html">\${product.ProductTitle}</a></h2>
                <div class="product-rate-cover">
                  <div class="product-rate d-inline-block">
                    <div class="product-rating" style="width: 90%"></div>
                  </div>
                ...
                <div class="product-card-bottom">
                  ...
                  <div class="add-cart">
                    <a id="addProductToCart" data-id="\${product._id}" data-category="\${product.Category}" class="addToCart" href="shop-cart.html"><i class="fi-rs-shopping-cart mr-5"></i>Add </a>
                  </div>
                </div>
              </div>
          </div>\`;
        productContainer.appendChild(productDiv);
            ...
            const deleteButtons = document.querySelectorAll('.addToCart');
    deleteButtons.forEach(button => {
      button.addEventListener('click', (event) => {
        ...
        fetch('http://localhost:8000/products/addToCart', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                },
                body: JSON.stringify({ productId: productId, Category: Category }),
                })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
...
app.use('/productsUpdate', require('./Routes/UpdateRoutes'))
